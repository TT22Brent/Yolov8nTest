...
class C2f(nn.Module):
    """Ghost-Enhanced C2f block using GhostBottleneck instead of Bottleneck."""

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        """
        Re-implemented C2f using GhostBottleneck blocks.

        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            n (int): Number of GhostBottleneck blocks.
            shortcut (bool): Placeholder for compatibility.
            g (int): Placeholder for compatibility.
            e (float): Expansion ratio.
        """
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)  # Split path
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # Output projection

        # Replace Bottleneck with GhostBottleneck
    ...
    def forward_split(self, x):
        y = self.cv1(x).split((self.c, self.c), dim=1)
        y = [y[0], y[1]]
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, dim=1))
...
class GhostC2f(nn.Module):
    """Ghost version of the C2f block using GhostBottleneck modules."""

    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        """
        GhostC2f with multiple GhostBottleneck layers.

        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            n (int): Number of GhostBottleneck blocks.
            shortcut (bool): (ignored here, for compatibility)
            g (int): (ignored here)
            e (float): Expansion ratio.
        """
        super().__init__()
    ...
    def forward(self, x):
        """Forward pass through GhostC2f block."""
