...
class TaskAlignedAssigner(nn.Module):
    ...
    def get_pos_mask(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, anc_points, mask_gt):
        """
        Add Big Sieve before Top-K to eliminate clearly bad anchors (e.g., IoU < 0.2).
        """
        ...
        # 🧹 Big Sieve: remove low-quality candidates before Top-K
        sieve_thresh = 0.2
        big_sieve_mask = (overlaps > sieve_thresh) * mask_gt * mask_in_gts  # shape: [B, N, H*W]
        align_metric = align_metric * big_sieve_mask  # ⚠️ 将低质量位置的指标变为0

        mask_topk = self.select_topk_candidates(align_metric)  # ✅ 不传 topk_mask
        mask_pos = mask_topk * big_sieve_mask

        return mask_pos, align_metric, overlaps

    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        """
        SimOTA-OC: IoU-only metric, ignore classification scores.
        """
        ...
        align_metric = overlaps.clone()  # 只用 IoU 作为匹配得分
